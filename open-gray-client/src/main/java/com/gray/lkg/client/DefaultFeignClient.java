 ...
 */
public class DefaultFeignClient extends LoadBalancerFeignClient {


    private final List<FeignInterceptor> feignInterceptorList;

    public DefaultFeignClient(Client delegate, CachingSpringLoadBalancerFactory lbClientFactory,
   ...
   private class LoadBalancerInterceptorChain extends CommonFeignInterceptorChain {
       ...
       LoadBalancerInterceptorChain(Request.Options options) {
           super(DefaultFeignClient.this.feignInterceptorList.iterator(),options);
       }

       @Override
       protected Response doExecute(Request request, Request.Options options) throws IOException {
           return DefaultFeignClient.super.execute(request, options);
       }
   }
}
